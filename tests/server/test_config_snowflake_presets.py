...
from helpers import (
    install_fake_snowflake_connect, login_as, write_database_presets_file, FAKE_DB_CONFIG_ENCRYPTION_KEY, FAKE_SESSION_SIGNING_KEY,
)
...
def test_preset_credential_never_appears_in_config_response(app_factory, tmp_path, monkeypatch):
    ...
    assert data['configured_databases'][0] == {"id": "snowflake+Sample Data", "name": "Sample Data", "type": "snowflake", "dialect_name": "Snowflake SQL"}
...
def test_anonymous_visitor_never_receives_the_presets_credential(app_factory, tmp_path, monkeypatch):
    ...
    assert resp.get_json()['configured_databases'] == [{"id": "snowflake+Sample Data", "name": "Sample Data", "type": "snowflake", "dialect_name": "Snowflake SQL"}]
