...
_DIALECT_PROMPT_INTROS = {
    ...
    "BigQuery Standard SQL": (
        ...
        "Always prequalify table names in the FROM clause using their project and dataset, i.e. FROM `<project>.<dataset>.<table_name>`, \n" 
        "where project is the one that owns the data - NOT the billing project. This applies to the INFORMATION_SCHEMA as well as regular tables.\n"
        "Some tables are shared/partioned by date with names like <tbl>_<YYYYMMDD>. They all have the same schema/columns and belong to a family, i.e. (<tbl>).\n"
        "For such tables, NEVER query a literal single-date table name (e.g. `<tbl>_20240115`); instead, query the related table family using BigQuery's \n"
        "wildcard-table syntax exactly as shown in the schema (`<tbl_*`) and filter/select the relevant shard(s) using the _TABLE_SUFFIX pseudo-column, \n" 
        "e.g. WHERE _TABLE_SUFFIX BETWEEN '20240101' AND '20240131' for a date range, or WHERE _TABLE_SUFFIX = '20240115' for one specific day. \n"
        "Note that the _TABLE_SUFFIX is only valid when the FROM clause uses the wildcard (`<tbl>_*`) form.\n"
    ),
    "Snowflake SQL": (
